...
import { CollectionData2, CollectionPluginData, getCollectionPlugins } from "../../components/collection/collectionState";
...
const useCollection = (props: useCollectionProps | null) => {
    ...
    const [collection, setCollection] = useState<CollectionData2 | null>(null);
    ...
    const getCollectionData2Account = useCallback(() => {
        if (!pageName) {
            setCollection(null);
    ...
    const fetchInitialCollectionData2 = useCallback(async () => {
        if (!check_initial_collection.current) {
            return;
        }

        let collection_account = getCollectionData2Account();
        ...
        const [collection] = CollectionData2.struct.deserialize(collection_data);
    ...
    const handleAccountChange = useCallback((accountInfo: any) => {
        ...
        const [updated_data] = CollectionData2.struct.deserialize(account_data);
    ...
    useEffect(() => {
        ...
        const collectionAccount = getCollectionData2Account();
        if (!collectionAccount) return;

        // Fetch the initial account data
        fetchInitialCollectionData2();
    ...
    }, [connection, pageName, fetchInitialCollectionData2, getCollectionData2Account, handleAccountChange]);
